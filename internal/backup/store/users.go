...
// UI's "Users" admin section (see SaveWebUIUser and friends below), distinct
// from the single config-file admin (webui.username/password, resolved by
// the config package) that alone can manage them. permissions is a
// permission.Permission bitmask (see the permission package).
...
// ErrWebUIUserExists is returned by SaveWebUIUser when username is already
// taken.
...
// HashPassword bcrypt-hashes password for storage (see SaveWebUIUser/
// UpdateWebUIUserPassword). bcrypt.DefaultCost balances hashing time against
...
// violation — modernc.org/sqlite doesn't export a typed error for this worth
// importing just for SaveWebUIUser's one check, so this matches on the
// driver's own error text instead.
func isUniqueConstraintErr(err error) bool {
	return strings.Contains(err.Error(), "UNIQUE constraint failed")
}

// SaveWebUIUser adds a new dashboard user granted perm, hashing password for
// storage (see HashPassword). Returns ErrWebUIUserExists if username is
// already taken.
func (s *Store) SaveWebUIUser(ctx context.Context, username, password string, perm permission.Permission) error {
	...
	if _, err := s.db.ExecContext(ctx, insert, username, hash, int(perm), time.Now().UTC()); err != nil {
...
func (s *Store) UpdateWebUIUserPassword(ctx context.Context, username, password string) error {
	hash, err := HashPassword(password)
	if err != nil {
		return err
	}

	res, err := s.db.ExecContext(ctx, `UPDATE webui_users SET password_hash = ? WHERE username = ?`, hash, username)
	if err != nil {
...
func (s *Store) UpdateWebUIUserPermissions(ctx context.Context, username string, perm permission.Permission) error {
	res, err := s.db.ExecContext(ctx, `UPDATE webui_users SET permissions = ? WHERE username = ?`, int(perm), username)
	if err != nil {
...
// doesn't exist.
func (s *Store) DeleteWebUIUser(ctx context.Context, username string) error {
	res, err := s.db.ExecContext(ctx, `DELETE FROM webui_users WHERE username = ?`, username)
	if err != nil {
...
func (s *Store) ListWebUIUsers(ctx context.Context) ([]WebUIUser, error) {
	query := `SELECT username, permissions, created_at FROM webui_users ORDER BY username`

	return queryRows(ctx, s.db, "listing web UI users", query, nil, func(rows *sql.Rows) (WebUIUser, error) {
		var (
			u    WebUIUser
			perm int
		)
...
// SaveWebUIUser), reporting ok=false if username doesn't exist.
func (s *Store) GetWebUIUser(ctx context.Context, username string) (WebUIUser, bool, error) {
	return queryRowOptional(ctx, s.db, fmt.Sprintf("looking up web UI user %q", username),
...
// user (see SaveWebUIUser), returning its granted permissions and ok=true
// only if username exists and password matches its stored hash.
...
// timing attack.
func (s *Store) VerifyWebUIUser(ctx context.Context, username, password string) (permission.Permission, bool, error) {
	type row struct {
		hash string
		perm int
	}

	r, ok, err := queryRowOptional(ctx, s.db, fmt.Sprintf("looking up web UI user %q", username),
